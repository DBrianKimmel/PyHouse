...
class UsbDriverAPI(Driver_USB.UsbDriverAPI):
    ...
    def read_device(self, p_usb):
        ...
        if g_debug > 5:
            print "Driver_USB_0403_6001.read_device() - Name:{0:}, Endpoint:{1:#02x}, Size:{2:}".format(p_usb.Name, p_usb.epi_addr, p_usb.epi_packet_size)
        try:
            l_msg = p_usb.Device.read(p_usb.epi_addr, p_usb.epi_packet_size, timeout = 400)
            l_len = len(l_msg)
            if l_len > 0:
                if g_debug > 4:
                    print "Driver_USB_0403_6001.read_device() - Len:{0:}, Msg:{1:}".format(l_len, l_msg)
                p_usb.msg_len += l_len
...
class API(UsbDriverAPI):
    ...
    def Start(self, p_controler_obj):
        if g_debug > 0:
            print "Driver_USB_0403_6001.Start() - Name:{0:}".format(p_controler_obj.Name)
