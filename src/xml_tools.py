...
def find_config_file():
    ...
    for l_dir in l_std_path:
        ...
        print "find-config checking dir: {0:}".format(l_dir)
        if not os.path.exists(l_dir):
            print "find-config -- No such path exists!"
            continue
        for l_name in l_std_name:
            print "find-config checking filename: {0:}".format(l_name)
            l_file = os.path.join(l_dir, l_name)
            if os.path.exists(l_file):
                print "find-config found file: {0:}".format(l_file)
                return l_file
    print "Find configfile - nothing found."
    return None
...
def create_empty_config_file(p_name):
    ...
    """
    print "Create Empty Config File"
    ...
    l_fn = open(os.path.expanduser(p_name), 'w')
