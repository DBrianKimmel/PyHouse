...
class HouseDialog(gui_tools.GuiTools):
    ...
    def __init__(self, p_parent, p_key, p_title = None):
        ...
        Entry(self.m_frame, textvar = self.Key, state = DISABLED).grid(row = 1, column = 1)
        ...
        Entry(self.m_frame, textvar = self.Name).grid(row = 3, column = 1)
        Label(self.m_frame, text = "Street").grid(row = 4, column = 0, sticky = E)
        Entry(self.m_frame, textvar = self.Street).grid(row = 4, column = 1)
