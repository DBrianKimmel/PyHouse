...
class LightingDialog(gui_tools.GuiTools):
    ...
    def __init__(self, p_parent, p_key, p_kind, p_title = None):
        ...
        Label(self.m_frame, text = "Family").grid(row = 3, column = 0)
        self.pulldown_box(self.m_frame, VAL_FAM, self.Family).grid(row = 3, column = 1)
        Label(self.m_frame, text = "Active").grid(row = 5, column = 0)
        self.yes_no_radio(self.m_frame, self.Active).grid(row = 5, column = 1)
        Label(self.m_frame, text = "Comment").grid(row = 7, column = 0)
        Entry(self.m_frame, textvar = self.Comment).grid(row = 7, column = 1)
        Label(self.m_frame, text = "Key").grid(row = 9, column = 0)
        Entry(self.m_frame, textvar = self.Key, state = DISABLED).grid(row = 9, column = 1)
        Label(self.m_frame, text = "Type").grid(row = 11, column = 0)
        Entry(self.m_frame, textvar = self.Type).grid(row = 11, column = 1)
        Label(self.m_frame, text = "Coords").grid(row = 13, column = 0)
        Entry(self.m_frame, textvar = self.Coords).grid(row = 13, column = 1)
        Label(self.m_frame, text = "Room").grid(row = 15, column = 0)
        Entry(self.m_frame, textvar = self.Room).grid(row = 15, column = 1)
        Label(self.m_frame, text = "Dimmable").grid(row = 17, column = 0)
        Entry(self.m_frame, textvar = self.Dimmable).grid(row = 17, column = 1)
        if l_type == 'Controller':
            Label(self.m_frame, text = 'Interface').grid(row = 31, column = 0)
        ...
        if p_title.startswith("Edit"):
            l_text = "Save"
            Button(self.m_frame, text = 'Delete', command = self.delete_entry).grid(row = 91, column = 1)
