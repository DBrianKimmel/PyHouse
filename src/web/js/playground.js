...
/*
 * Lets users play whatever games, contains the whole startup logic for the site.
 * This is the client side part of the basis of this whole website.
 * If you plan for yet another sequencing of events on startup then please change the code here.
 * 
 * The actual implementation starts, waits for all images to be loaded (assuming that all images are loaded before user interaction is allowed.
 ...
 * Insofar, as the insertion happens through the basic insertion method within the deliverred pages html,
 * namely <nevow:invisible nevow:render="playground" /> which pumps up a standard livepage with exactly one fragment inserted.
 * After initialisation Divmod.Runtime.theRuntime. issues a call via the preset vector which contains globals.playground.appStartup you'll find below.
 * This call then starts the pulling in of whatever is needed.
 * Please see additional comments below. 
 */

//Nevow.Athena.Widget.subclass(playground, 'Playground').methods(
helpers.Widget.subclass(playground, 'Playground').methods(
	...
	// Do whatever needs to be started up, checking for images loading and other gory stuff.
	...
	// trials with processing for mobile webapps.
	//
	...
	function appStartup(self) {
		...
		function ready() { // we're now ready for action
			...
			d1.addErrback(function(res) {
				alert("playground.addErrback res = " + res)
