 ...
 */
function createNewButtonWindow(p_name) {    
	var divId = createNewWindow(167, 300, 250, 200, 250);  // id, width, height, start x, start y
	var content = fillNewButtonWindow(divId, p_name);
 ...
 */
function createChangeButtonWindow(p_json) {    
	var divId = createNewWindow(167, 300, 220, 100, 250);  // id, width, height, start x, start y
	var content = fillChangeButtonWindow(divId, p_json);
	document.getElementById('good_windowContent' + divId).innerHTML = content;
	}
 ...
 */
function fillNewButtonWindow(p_id, p_name) {
	//alert('fill new buttonpage (add)');
	var ret =	'';
 ...
 */
function fillChangeButtonWindow(p_divid, p_json) {
	alert('fill new buttonpage (change) JSON=' + p_json);
