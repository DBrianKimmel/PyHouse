...
from Modules.Core.data_objects import LightData, UuidData, GarageDoorData
...
LOG = Logging.getLogger('PyHouse.LightingGrgDr  ')
...
class API(object):
    ...
    @staticmethod
    def read_all_GarageDoors_xml(p_pyhouse_obj, p_door_sect_xml):
        """
        @param p_pyhouse_obj: is the master information store
        @param p_door_sect_xml: the "GarageDoorSection" of the config
        ...
        l_dict = {}
        try:
            for l_xml in p_door_sect_xml.iterfind('GarageDoor'):
                l_obj = Utility._read_one_door_xml(p_pyhouse_obj, l_xml)
