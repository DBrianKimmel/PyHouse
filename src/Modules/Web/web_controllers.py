...
class ControllersElement(athena.LiveElement):
    ...
    def saveControllerData(self, p_json):
        ...
        if l_delete:
            try:
                del self.m_pyhouse_obj.House.Controllers[l_controller_ix]
            except AttributeError:
                print("web_controllers - Failed to delete - JSON: {0:}".FORMAT(l_json))
            return
        try:
            l_obj = self.m_pyhouse_obj.House.Controllers[l_controller_ix]
        except KeyError:
        ...
        self.m_pyhouse_obj.House.Controllers[l_controller_ix] = l_obj
