...
class HouseWindow(GuiTools):
    ...
    m_ix = 0
    m_houses_dict = None
    m_house_add_flag = None
    m_room_add_flag = False
    ...
    def show_all_houses(self, p_gui_obj):
        ...
        l_max = self.add_house_buttons(p_gui_obj)
        Button(p_gui_obj.ModuleMenuFrame, text = "ADD New House", fg = FG, bg = BG_BOTTOM,
               command = lambda x = p_gui_obj, y = l_max + 1: self.add_house(x, y)
    ...
    def add_house_buttons(self, p_gui_obj):
        """Place the house buttons in the menu.
        """
        if g_debug >= 2:
            print "gui_house.add_house_buttons()", self.m_houses_dict
        l_house = []
        l_max_house_ix = -1
...
#============================
#
# House selection Button Handlers
#
#============================

    def add_house(self, p_gui_obj, p_ix):
        """Add a new house to the houses obj.
        pass the real work into the house dialog.
        """
        self.m_house_add_flag = True
        l_name = ''
        l_house_obj = HouseData()
        l_house_obj.Name = l_name
        l_house_obj.Key = p_ix
        if g_debug > 2:
            print "gui_house.add_house() - Key:{0:}".format(self.m_ix), self.m_house_add_flag
        HouseDialog(p_gui_obj, l_house_obj, self.m_houses_dict, True, "Adding House")

    def edit_house(self, p_gui_obj, p_house_obj, p_key):
        self.m_house_add_flag = False
        if g_debug > 2:
            print "gui_house.edit_house() - Name:{0:}, Key:{1:} ".format(p_house_obj.Name, p_key), self.m_house_add_flag
        HouseDialog(p_gui_obj, p_house_obj, self.m_houses_dict, False, "Editing House")
...
class HouseDialog(HouseWindow):
    ...
    def __init__(self, p_gui_obj, p_house_obj, p_houses_dict, p_adding, p_title = None):
        self.m_houses_dict = p_houses_dict
        if g_debug > 0:
            print "gui_house.HouseDialog.__init__() - Show add/edit house dialog", self.m_house_add_flag, p_house_obj
        self.m_house_obj = p_house_obj
        self.m_house_add_flag = p_adding
    ...
    def show_house_dialog(self, p_gui_obj, p_house_obj):
        if g_debug >= 2:
            print "gui_house.show_house_dialog() AddFlag:{0:}".format(self.m_house_add_flag)
        ...
        l_nam = self.get_entry_str(p_gui_obj.ModuleDialogFrame, 3, 'Name', self.Name, width = 50)
        l_nam.focus_force()
        ...
        l_max = self.add_all_rooms(p_gui_obj, p_house_obj)
        if self.m_house_add_flag:
            l_cmd = lambda x = p_gui_obj, y = p_house_obj: self.add_house_d(x, y)
            l_add = Button(p_gui_obj.ModuleDialogFrame, text = 'Add this House', fg = "blue", bg = BG_BOTTOM,
                   command = lambda x = p_gui_obj, y = p_house_obj: self.add_house_d(x, y))
            l_add.grid(row = 91, column = 0)
            l_add.bind('<Return>', l_cmd)
        else:
            ...
            Button(p_gui_obj.ModuleDialogFrame, text = 'Update House', fg = "blue", bg = BG_BOTTOM,
                   command = lambda x = p_gui_obj, y = p_house_obj, z = self.m_houses_dict: self.save_house(x, y, z)).grid(row = 91, column = 0)
    ...
    def add_all_rooms(self, p_gui_obj, p_house_obj):
        ...
        """
        if g_debug >= 9:
            print "gui_house.add_all_rooms() - {0:}".format(p_house_obj.Name)
...
#============================
    ...
    def add_house_d(self, p_gui_obj, p_house_obj):
        """Merge the location(in vars) into the house
        Create new entry in houses and restart PyHouse
        TODO: add a house
        """
        if g_debug >= 3:
            print "gui_house.add_house_d() - Adding new house with Key:{0:} into {1:}".format(p_house_obj.Key, self.m_houses_dict)
        l_api = house.API()
        p_house_obj = self.save_house_vars(p_house_obj)
        l_houses_obj = HousesData()
        l_houses_obj.Name = p_house_obj.Name
        l_houses_obj.Key = p_house_obj.Key
        l_houses_obj.Object = p_house_obj
        l_houses_obj.HouseAPI = l_api
        self.m_houses_dict[p_house_obj.Key] = l_houses_obj
        if g_debug >= 3:
            print "gui_house.add_house_d() - Name:{0:}, Key:{1:}".format(p_house_obj.Name, p_house_obj.Key), self.m_houses_dict
    ...
    def save_house(self, p_gui_obj, p_house_obj, p_houses_obj):
        """Merge the location(in vars) into the house
        """
    ...
    def add_room(self, p_gui_obj, p_house_obj, p_room_key):
        ...
        RoomDialog(p_gui_obj, p_house_obj, p_room_key, self.m_houses_dict, "Adding Room")

    def edit_room(self, p_gui_obj, p_house_obj, p_room_key):
        self.m_room_add_flag = False
        if g_debug >= 2:
            print "gui_house.edit_room() # ", p_room_key
        RoomDialog(p_gui_obj, p_house_obj, p_room_key, self.m_houses_dict, "Editing Room")
...
class RoomDialog(HouseDialog):
    ...
    def __init__(self, p_gui_obj, p_house_obj, p_room_key, p_houses_dict, p_title = None):
        """
        ...
        l_nam = self.get_entry_str(p_gui_obj.SecondDialogFrame, 3, 'Name', self.Name, width = 50)
        l_nam.focus_force()
    ...
    def get_room_obj(self, p_house_obj, p_room_key):
        """Get existing room(Edit) or create an empty room object(Add)
...
#============================
    ...
    def quit_room_dialog(self, p_gui_obj, p_house_obj):
        ...
        Do not update the room
        """
        if g_debug >= 2:
            print "gui_house.quit_room_dialog()", p_house_obj
        self.frame_delete(p_gui_obj.ModuleDialogFrame)
        self.frame_delete(p_gui_obj.SecondDialogFrame)
        p_gui_obj.SecondDialogWindow.destroy()
        self.show_house_dialog(p_gui_obj, p_house_obj)


class InternetDialog(GuiTools):
    """
    Add / edit / delete dialog window for a room.
    """
