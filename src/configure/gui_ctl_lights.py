...
class CtlLightsWindow(GuiTools):
    ...
    def ctl_lights(self, p_obj, p_key):
        """
        @param p_obj: is the house object
        @param p_key: the key to look up the light info.
        """
        if g_debug > 0:
            print "Control lights", p_obj.Name
        LightingDialog(self.m_lights_frame, p_obj, p_key, "Editing Light")
...
class LightingDialog(gui_tools.GuiTools):
    ...
    @param p_title:
    """
    def __init__(self, p_parent, p_obj, p_key, p_title = None):
        ...
        self.l_result = None
        self.create_vars()
        l_obj = self.load_vars(p_obj, p_key)
        l_res = 100
        ...
        Button(self.m_frame, text = 'Change', fg = "blue", bg = gui_tools.BG_BOTTOM, command = lambda x = p_obj, y = p_key: self.change_light(x, y)).grid(row = 91, column = 0)
    ...
    def load_vars(self, p_obj, p_key):
        """
        @param p_obj: is the house object.
        @param p_key: is the index of the light to control
        """
        if g_debug > 0:
            print "gui_ctl_lights.load_vars() ", p_obj, p_key
        l_obj = p_obj.Lights[p_key]
        l_obj.Key = p_key
    ...
    def change_light(self, p_obj, p_key):
        """
        ...
        l_key = self.Key.get()
        l_obj = p_obj.Lights[l_key]
        if g_debug > 0:
            print "gui_ctl_lights.change_light() - Name:{0:}, Level:{1}, Key:{2:} ".format(l_light, l_level, p_key), p_obj, ';  ', l_obj
        lighting.LightingUtility().change_light_setting(p_obj, p_key, l_level)
