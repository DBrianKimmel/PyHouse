...
class UPnPPublisher(resource.Resource, log.Loggable):
    ...
    def decode_result(self, element):
        l_type = element.get('{http://www.w3.org/1999/XMLSchema-instance}type')
        if l_type is not None:
            try:
                prefix, local = l_type.split(":")
                if prefix == 'xsd':
                    l_type = local
            except ValueError:
                pass
        if l_type == "integer" or l_type == "int":
            return int(element.text)
        if l_type == "float" or l_type == "double":
