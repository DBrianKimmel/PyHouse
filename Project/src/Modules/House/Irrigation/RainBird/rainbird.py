...
class RainbirdEncryption:
    ...
    def AddPadding(self, p_data):
        l_data = p_data
        l_len = len(l_data)
        remaining_len = self.BLOCK_SIZE - l_len
        l_pad_len = remaining_len % self.BLOCK_SIZE
        pad_string = self.PAD * l_pad_len
        return ''.join([l_data, pad_string])
