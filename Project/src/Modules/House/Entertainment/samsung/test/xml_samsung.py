"""
@name:      PyHouse/src/Modules/Housing/Entertainment/samsung/_test/xml_samsung.py
@author:    D. Brian Kimmel
@contact:   D.BrianKimmel@gmail.com
@copyright: (c) 2018-2018 by D. Brian Kimmel
@note:      Created on Mar 19, 2018
@license:   MIT License
@summary:


<?xml version="1.0" ?>
<SamsungSection Active="True">
    <Device Active="True" Key="0" Name="L/R-TV ">
        <UUID>Samsung.-0000-0000-0000-0123456789ab</UUID>
        <Comment>CT - 48in Smart-Tv 48abc1234 </Comment>
        <Installed>2016-07-29</Installed>
        <IPv4>192.168.9.118</IPv4>
        <Model>UN48J5201AFXZA</Model>
        <Port>55000</Port>
        <RoomName>Living Room</RoomName>
        <RoomUUID>Room....-0000-0000-0000-0123456789ab</RoomUUID>
        <Type>TV</Type>
        <Volume>53</Volume>
    </Device>
</SamsungSection>

"""

__updated__ = '2018-10-27'

TESTING_SAMSUNG_SECTION = 'SamsungSection'
TESTING_DEVICE = 'Device'
TESTING_SAMSUNG_ACTIVE = 'True'
TESTING_SAMSUNG_TYPE = 'Component'

L_SAMSUNG_SECTION_START = '<' + TESTING_SAMSUNG_SECTION + ' Active="' + TESTING_SAMSUNG_ACTIVE + '">'
L_SAMSUNG_SECTION_END = '</' + TESTING_SAMSUNG_SECTION + '>'
L_DEVICE_END = '</' + TESTING_DEVICE + '>'
L_SAMSUNG_SECTION_TYPE = '<Type>' + TESTING_SAMSUNG_TYPE + '</Type>'

TESTING_SAMSUNG_DEVICE_NAME_0 = 'L/R-TV'
TESTING_SAMSUNG_DEVICE_KEY_0 = '0'
TESTING_SAMSUNG_DEVICE_ACTIVE_0 = 'True'
TESTING_SAMSUNG_DEVICE_UUID_0 = 'Samsung.-0000-0000-0000-0123456789ab'
TESTING_SAMSUNG_DEVICE_COMMAND_SET_0 = '2015-A'
TESTING_SAMSUNG_DEVICE_COMMENT_0 = 'CT - 48in Smart-Tv 48abc1234 '
TESTING_SAMSUNG_DEVICE_INSTALLED_0 = '2016-07-29'
TESTING_SAMSUNG_DEVICE_IPV4_0 = '192.168.9.118'
TESTING_SAMSUNG_DEVICE_MODEL_0 = 'UN48J5201AFXZA'
TESTING_SAMSUNG_DEVICE_PORT_0 = '55000'
TESTING_SAMSUNG_DEVICE_ROOM_NAME_0 = 'Living Room'
TESTING_SAMSUNG_DEVICE_ROOM_UUID_0 = 'Room....-0000-0000-0000-0123456789ab'
TESTING_SAMSUNG_DEVICE_TYPE_0 = 'TV'
TESTING_SAMSUNG_DEVICE_VOLUME_0 = '53'

L_SAMSUNG_DEVICE_START_0 = \
    '<' + TESTING_DEVICE + \
    ' Name="' + TESTING_SAMSUNG_DEVICE_NAME_0 + \
    '" Key="' + TESTING_SAMSUNG_DEVICE_KEY_0 + \
    '" Active="' + TESTING_SAMSUNG_DEVICE_ACTIVE_0 + \
    '">'
L_SAMSUNG_UUID_0 = '<UUID>' + TESTING_SAMSUNG_DEVICE_UUID_0 + '</UUID>'
L_SAMSUNG_COMMENT_0 = '<Comment>' + TESTING_SAMSUNG_DEVICE_COMMENT_0 + '</Comment>'
L_SAMSUNG_COMMAND_SET_0 = '<CommandSet>' + TESTING_SAMSUNG_DEVICE_COMMAND_SET_0 + '</CommandSet>'
L_SAMSUNG_INSTALLED_0 = '<Installed>' + TESTING_SAMSUNG_DEVICE_INSTALLED_0 + '</Installed>'
L_SAMSUNG_IPV4_0 = '<IPv4>' + TESTING_SAMSUNG_DEVICE_IPV4_0 + '</IPv4>'
L_SAMSUNG_MODEL_0 = '<Model>' + TESTING_SAMSUNG_DEVICE_MODEL_0 + '</Model>'
L_SAMSUNG_PORT_0 = '<Port>' + TESTING_SAMSUNG_DEVICE_PORT_0 + '</Port>'
L_SAMSUNG_ROOM_NAME_0 = '<RoomName>' + TESTING_SAMSUNG_DEVICE_ROOM_NAME_0 + '</RoomName>'
L_SAMSUNG_ROOM_UUID_0 = '<RoomUUID>' + TESTING_SAMSUNG_DEVICE_ROOM_UUID_0 + '</RoomUUID>'
L_SAMSUNG_TYPE_0 = '<Type>' + TESTING_SAMSUNG_DEVICE_TYPE_0 + '</Type>'
L_SAMSUNG_VOLUME_0 = '<Volume>' + TESTING_SAMSUNG_DEVICE_VOLUME_0 + '</Volume>'

L_SAMSUNG_DEVICE_0 = '\n'.join([
    L_SAMSUNG_DEVICE_START_0,
    L_SAMSUNG_UUID_0,
    L_SAMSUNG_COMMENT_0,
    L_SAMSUNG_COMMAND_SET_0,
    L_SAMSUNG_INSTALLED_0,
    L_SAMSUNG_IPV4_0,
    L_SAMSUNG_MODEL_0,
    L_SAMSUNG_PORT_0,
    L_SAMSUNG_ROOM_NAME_0,
    L_SAMSUNG_ROOM_UUID_0,
    L_SAMSUNG_TYPE_0,
    L_SAMSUNG_VOLUME_0,
    L_DEVICE_END
])

XML_SAMSUNG_SECTION = '\n'.join([
    L_SAMSUNG_SECTION_START,
    L_SAMSUNG_SECTION_TYPE,
    L_SAMSUNG_DEVICE_0,
    L_SAMSUNG_SECTION_END
])

# ## END DBK
